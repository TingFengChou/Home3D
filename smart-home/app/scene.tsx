...
import {Wind,CircleDot,Monitor,RotateCcw,Plus,Minus,Layers,Blinds,Thermometer} from 'lucide-react';
...
export default function RoomScene(props:Props){
 ...
 const items=[{id:'purifier' as const,name:'空氣清淨機',icon:Wind,status:props.state.purifier.on?`${props.state.purifier.fan}% 風速`:'已關閉'},{id:'vacuum' as const,name:'掃地機器人',icon:CircleDot,status:props.state.vacuum.mode==='cleaning'?'清掃中':props.state.vacuum.mode==='paused'?'已暫停':props.state.vacuum.mode==='returning'?'回充中':'待命'},{id:'curtain' as const,name:'智慧窗簾',icon:Blinds,status:`開啟 ${Math.round(props.state.curtain.opening)}%`},{id:'ac' as const,name:'智慧空調',icon:Thermometer,status:props.state.ac.on?`${props.state.ac.temperature}°C · 運作中`:'已關閉'},{id:'hub' as const,name:'Google Nest Hub',icon:Monitor,status:'家庭中樞'}];
